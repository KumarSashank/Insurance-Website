...
(function ($) {
  ...
  if ($(".lazy-img").length) {
    var lazyOptions = {
      effect: "fadeIn",
      threshold: $(window).width() < 768 ? 100 : 300, // Adjust threshold for mobile devices
    };
    $(".lazy-img").Lazy(lazyOptions);
  }
  ...
  $(window).on("load", function () {
    ...
    // ------------------------------- Scroll Animation
    var isMobile = $(window).width() < 768; // Check if the device is mobile

    var wow = new WOW({
      boxClass: "wow", // animated element css class (default is wow)
      animateClass: "animated", // animation css class (default is animated)
      offset: isMobile ? 50 : 0, // distance to the element when triggering the animation (customized for mobile)
